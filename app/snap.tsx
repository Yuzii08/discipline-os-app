...
import { Camera, RefreshCw, X, CheckCircle, Sparkles, Play } from 'lucide-react-native';
...
export default function SnapScreen() {
  ...
  const handleFinishProtocol = async () => {
    ...
    if (compId) {
       await supabase.from('mission_completions')
        .update({ 
          end_image_url: finishB64, 
          ended_at: new Date().toISOString()
        } as any)
        .eq('completion_id', compId);
    }

    await executeAIAudit(finishB64);
  };

  const executeAIAudit = async (finishB64: string) => {
    if (!compId) {
       setIsProcessing(false);
       return;
    }
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('start_image_url, is_grace_period, missions!inner(base_reward_points, wager_amount)')
        .eq('completion_id', compId)
        .single();
        
      if (error || !data) throw error || new Error("No completion record found");

      const startBase64 = data.start_image_url;
  ...
  if (machineState === 'FAIL') {
    return (
      <View style={[styles.root, styles.center, { backgroundColor: '#F5E6E6' }]}>
    ...
    <View style={styles.root} pointerEvents={isProcessing ? 'none' : 'auto'}>
      ...
      <View style={styles.cameraWrap}>
        ...
        {isProcessing && machineState === 'PENDING' && (
          <View style={[StyleSheet.absoluteFill, styles.center, { backgroundColor: `rgba(249, 247, 242, 0.9)`, zIndex: 999 }]}>
            <ActivityIndicator size="large" color={SAGE} />
...
const styles = StyleSheet.create({
  ...
  camera:     { flex: 1 },
  clayLifted: { shadowColor: '#B8B2A5', shadowOffset: { width: 6, height: 8 }, shadowOpacity: 0.4, shadowRadius: 16, elevation: 8, borderWidth: 1, borderColor: 'rgba(255,255,255,0.7)' },
  ...
  hintBox: { position: 'absolute', bottom: 20, alignSelf: 'center', backgroundColor: 'rgba(61,64,91,0.6)', borderRadius: 100, paddingHorizontal: 16, paddingVertical: 6 },
  hintText: { fontSize: 11, fontWeight: '700', color: '#fff', letterSpacing: 0.5 },
